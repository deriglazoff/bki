...
Использование: каждое правило из [och-card.md](och-card.md) должно попадать хотя бы в один кейс ветки; при изменении правила прогнать ветку и сверить expected action. Агент `solve-osparivanie` берёт формулировки отсюда как регрессию тона, не как поиск «похожих» по ФИО.
