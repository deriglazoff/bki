...
- Слой A (Исх): адресат + блок предмета. Кредит `{UID}` → **ОЧ**. Фразы ЗЗ (формула согласия / «выполнил запрос кредитного отчета» / «Заявка / обращение») → **ЗЗ**. Канцелярия НБКИ (GUTDF, «включая информацию из титульной части», «Удаление информации о запросах производится в порядке, предусмотренном ранее», ст. 15.26.3, 10 дней) — не предмет. Подтипы ОЧ из одного слота «Кредит» не ставить.
